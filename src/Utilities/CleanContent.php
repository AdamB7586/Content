    ...
    private function validateImages()
    {
        ...
        if (is_array($imageFind) && !empty($imageFind)) {
            foreach ($imageFind as $images) {
                ...
                if (!$images->width && $this->css['width']) {
                    $images->width = str_replace('px', '', $this->css['width']);
                }
                if (!$images->height && $this->css['height']) {
                    $images->height = str_replace('px', '', $this->css['height']);
                }
                if ($this->css['float']) {
                    if ($images->class) {
                        $images->class.= ' '.$this->css['float'];
                    } else {
                        $images->class = $this->css['float'];
                    }
                }
